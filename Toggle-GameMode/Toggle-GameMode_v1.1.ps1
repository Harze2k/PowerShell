...
function ToggleGameMode {
    ...
    if ($turbo) {
        $excludedProcesses = @('pwsh', 'XtuService', 'pcee4', 'ROCCAT_Swarm_Monitor') 
    }
    ...
    $servicesToStop = @(
        ...
        'Windows Search', 'Windows Time', 'Windows Update', 'WMI Performance Adapter', 'Xbox Accessory Management Service', 'Xbox Live Auth Manager', 'Xbox Live Game Save', 'Xbox Live Networking Service', 'GraphicsPerfSvc'
    )
    ...
    switch ($Mode) {
        ...
        'Enable-GameMode' {
            ...
            Write-Host "There was $runningServicesBefore running services before, now its $((Get-Service -ErrorAction SilentlyContinue | Select-Object Status | Where-Object { $_.Status -eq 'Running' }).Count)." -ForegroundColor Green
            Write-Host "Stopped $servicesStopped/$($servicesToStop.count) unnecessary services." -ForegroundColor Green
            Write-Host "Stopping all the $servicesStopped services tock $($executionTimeServices.TotalSeconds) seconds" -ForegroundColor Green
